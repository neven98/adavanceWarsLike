...
public class BlueSoldier extends Unity {
    ...
    public BlueSoldier(int x,int y){
        super();
        Unity(x,y);
        posx = x+32;
        posy = y+32;
    ...
    public static void crea(){
        tex = new Texture(Gdx.files.internal("SoldatBlueAni.png"));
    ...
    public static void create(){
        ...
        if (Moved & (Gdx.input.isButtonPressed(Input.Buttons.LEFT))){
            if (Tile.map[Gdx.input.getX()/64][(1025-Gdx.input.getY())/64] == 3){
                if((checkX() || checkY()) && (checkXt() || checkYt()) && ( checkXs() || checkYs()) && (checkXvr()  || checkYvr()) && (checkXv() || checkYv()) ) {
    ...
    public static void DrawUnit(SpriteBatch batch, BitmapFont font){
        font.draw(batch, ""+BlueSoldier.life +"", posxFont, posyFont);
